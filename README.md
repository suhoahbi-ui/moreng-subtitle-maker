...
- 번역 기본 모델은 요청 수를 줄이기 위해 `gemini-2.5-flash-lite`를 사용하며, 필요 시 다른 Gemini Flash 모델로 전환을 시도합니다.
...
### Q. `429 RESOURCE_EXHAUSTED` 또는 quota exceeded가 나오면 어떻게 하나요?

사용 중인 Google/Gemini 프로젝트의 무료 티어 요청 한도나 사용량 제한에 걸린 상태입니다. 앱은 다른 Gemini Flash 모델로 전환을 시도하지만, 계속 실패하면 잠시 후 다시 시도하거나 Google AI Studio의 사용량/결제 설정을 확인해야 합니다.
